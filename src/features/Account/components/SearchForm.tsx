...
const DEFAULT_VALUES: Partial<SearchAccountParams> = {
  searchText: "",
};

const SearchForm = ({ onRefresh }: { onRefresh?: () => void }) => {
  const methods = useForm<SearchAccountParams>({
    defaultValues: DEFAULT_VALUES as SearchAccountParams,
  });
  ...
  useEffect(() => {
    ...
    reset(defaultValues as SearchAccountParams);
  }, [queryParams, reset]);
