...
const DEFAULT_VALUES: Partial<SearchSupplierParams> = {
  searchText: "",
};

const SearchForm = ({ onRefresh }: { onRefresh?: () => void }) => {
  const methods = useForm<SearchSupplierParams>({
    defaultValues: DEFAULT_VALUES as SearchSupplierParams,
  });
  ...
  useEffect(() => {
    const defaultValues: Partial<SearchSupplierQueryParams> = {
      ...DEFAULT_VALUES,
      page: queryParams.page,
      pageSize: queryParams.pageSize,
      searchText: (queryParams.searchText as string) || "",
    };
    reset(defaultValues as SearchSupplierParams);
  }, [queryParams, reset]);
